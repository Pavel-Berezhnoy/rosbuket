...
const SuccessMessage = ({ children }) => {
    const [openedState, setOpenedState] = useState(OpenModal.openedState);
    if (openedState?.open) {
    ...
    if (openedState?.open) {
        ...
        styles.splice(styles.indexOf('opacity-100'), 1, 'opacity-0');
        styles.splice(styles.indexOf('z-10'), 1, '-z-10');
    }
    return (
        <OpenModal.Provider value={{ openedState, setOpenedState }}>
            <div className={styles.join(" ")}>
                <div className="flex flex-row items-center border-b sm:border-b-0 w-full sm:w-auto pb-4 sm:pb-0">
                    ...
                    <div className="text-lg font-medium ml-3"><span>{openedState?.text}</span></div>
                </div>
            </div>
            {children}
        </OpenModal.Provider>
