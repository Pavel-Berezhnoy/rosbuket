...
const UpdateOrder = () => {
    ...
    const submitHandle = async (e) => {
        e.preventDefault();
        const response = await api.postJson("/api/admin/orders?_method=PUT", order);
        if (response.status === 200) message.setOpened(true);
    }
    return (
        <>
            <SuccessMessage text={"Сообщение отправлено пользователю на почту!"} opened={message.opened}></SuccessMessage>
